...
public class CoredataDAOImpl implements CoredataDAO {
    ...
    public List<Coredata> getCoredataByTaxonkeys(List<Integer> taxonkeys, int sinceYear, int untilYear) {
        ...
        //long time = end - start;
        //System.err.println("CoredataDAOImpl.getCoredataByTaxonkeys: " + time);
        return coredata;
    }
    ...
    public List<Integer> getAllTaxonkeys(int sinceYear, int untilYear) {
        //System.out.println("retrieving all taxonkeys in range " + sinceYear + " - " + untilYear);
        Session session = this.sessionFactory.getCurrentSession();
        String q = "select taxonkey from Coredata where cyear >= :syear and cyear <= :uyear";
        Query query = session.createQuery(q);
