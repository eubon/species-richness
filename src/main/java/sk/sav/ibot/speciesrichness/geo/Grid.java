...
public class Grid {
    ...
    /**
     * Creates layers of cells by years. Each cell contains total number of occurences and species
     * for all the data occuring within an area covered by specific cell in specific year.
     * Retrieve results by calling getLayers();
     * @param spatialResolution Width and height of cells contained in the grid
     * @param temporalResolution Which years include to the result, e.g. if years = [2001, 2002, 2003, 2004, 2005, 2006] and temporalResolution = 3 then result = [2001, 2004]
    ...
    public void occurencesInGrid(int spatialResolution, int temporalResolution, int startYear, List<Coredata> data) {
        for (Coredata d : data) {
            ...
            if ((d.getCyear() - startYear) % temporalResolution == 0) {
                ...
                //make cells into layers by years
    ...
    /**
     * Creates a new cell based on a point, and width and height of a cell in degrees.
     * The point belongs to the area covered by the cell. Borders of a cell are positioned in a way
     * that all previous cells, starting from the bottom-left corner of the grid have the same size
     ...
     * @return Cell with given width, containing the point, and existing in a year
     */
